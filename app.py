...
def main():
    ...
    with st.sidebar:
        ...
        total_capital = st.number_input(
            ...
            step=1000000,
            help="터틀 트레이딩을 위한 총 투자 가능 금액을 입력하세요"
        )
        ...
        if st.session_state.get('user_positions'):
            ...
            if not active_positions.empty:
                ...
                # 자금 사용률 경고
                if usage_rate > 80:
                    st.error("⚠️ 자금 사용률이 80%를 초과했습니다!")
                elif usage_rate > 60:
                    st.warning("⚠️ 자금 사용률이 60%를 초과했습니다.")
            else:
                st.metric("사용중인 자금", "0원")
                st.metric("남은 투자금", f"{total_capital:,}원")
        else:
